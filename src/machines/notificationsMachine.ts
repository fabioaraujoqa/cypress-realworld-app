...
export const notificationsMachine = dataMachine("notifications").withConfig({
  services: {
    fetchData: async (ctx, event: any) => {
      ...
      const resp = await httpClient.get(`https://cypress-realworld-app-puce.vercel.app/notifications`, {
    ...
    updateData: async (ctx, event: any) => {
      ...
      const resp = await httpClient.patch(
        `https://cypress-realworld-app-puce.vercel.app/notifications/${payload.id}`,
        payload
      );
      return resp.data;
    },
